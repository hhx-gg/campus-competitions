...
def build_requirement_rows():
    return [
        ...
        ("SEA-003", "P0", "专业、状态、对象、主办方类型、难度、含金量、收藏、形式、费用、年份、审核状态可组合筛选。", "覆盖单条件、交叉条件、重置和空结果。", "筛选测试报告", "", "", "□通过 □失败"),
        ...
        ("LOC-001", "P0", "收藏、隐藏、备注和计划均本地持久化，重启不丢失。", "写入数据、结束进程并重启验证。", "持久化测试日志", "", "", "□通过 □失败"),
        ...
        ("MIG-001", "P0", "数据迁移前自动备份；迁移失败恢复收藏、备注、计划和隐藏状态。", "注入迁移失败并核对前后数据哈希。", "迁移与恢复报告", "", "", "□通过 □失败"),
        ("SEC-001", "P0", "外链仅允许已登记HTTPS官方域名；非法协议和未登记域名被阻止。", "测试http、file、javascript及未知域名。", "安全测试报告", "", "", "□通过 □失败"),
        ("PRI-001", "P0", "收藏、备注、计划和隐藏状态不上传。", "抓包并检查应用数据流。", "网络抓包报告", "", "", "□通过 □失败"),
...
def build_document():
    ...
    metadata = [
        ...
        ("验收对象", "Windows 10/11 x64安装包、生产数据包与更新服务"),
    ...
    add_body(doc, "本文件定义《杜绝信息差——大学生友好竞赛消息工具》的最高等级发布验收标准。验收对象包括桌面应用、中央抓取与审核链路、签名数据包、自动更新服务、安装包及发布文档。")
    ...
    gates = [
        ...
        ("G4", "安全合规", "签名、回滚、隐私和外链白名单全部通过。", "任一失败即阻断"),
    ...
    for text in [
        "候选安装包、生产数据包和更新服务的版本已冻结，验收期间不允许无记录变更。",
    ...
    scenarios = [
        ...
        ("FUN-05", "参赛计划", "创建计划、任务、截止日和完成状态，重启后编辑。", "完整持久化。"),
        ("FUN-07", "日历", "跨月查看报名、初赛、复赛、决赛和结果节点。", "节点、筛选、今天与反向跳转全部可用。"),
        ("FUN-08", "数据更新", "发布新数据包并修改一条已核验竞赛日期。", "签名验证通过；显示变更；旧日期可查。"),
